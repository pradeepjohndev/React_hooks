    ...
    <aside className="h-screen shrink-0">
      <nav className="h-full flex flex-col bg-white border-r shadow-sm overflow-hidden">
        <div className="p-4 pb-2 flex justify-between items-center">
          <img src={logo} alt="Logo" className={`overflow-hidden transition-all ${expanded ? "w-10" : "w-0"}`} />
          <button onClick={() => setExpanded(c => !c)} className="p-1.5 rounded-lg bg-gray-50 hover:bg-gray-100">
...
export function SidebarItem({ icon, text, active, alert }) {
  ...
  return (
    <li className={`relative flex items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer transition-colors group ${
      active ? "bg-gradient from-indigo-200 to-indigo-100 text-indigo-800" : "hover:bg-indigo-50 text-gray-600"
    }`}>
      {icon}
      <span className={`whitespace-nowrap overflow-hidden transition-all ${expanded ? "w-52 ml-3" : "w-0 pointer-events-none"}`}>
        {text}
      </span>

      {alert && (
        <div className={`absolute right-2 top-1/2 -translate-y-1/2 w-2 h-2 rounded bg-indigo-400 ${expanded ? "" : "top-2 translate-y-0"}`} />
      )}
